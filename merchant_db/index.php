<?php
	...
	{
		$order_cols = array("amount"=>"amount", "price"=>"price", "name"=>"name");
	...
	if(!empty($_POST['name']))
	{
		...
		{
			$result = true;
			$count = $db->select("vending","`nameid` in ({$items})", "", "count(*) as cnt");
			$pages->items_total = $count[0]->cnt;
			$pages->mid_range = 5;  
			$pages->paginate();	
			
			$results = $db->select("vending left join `char` on `char`.char_id=`vending`.char_id","`nameid` in ({$items}) ORDER BY {$order} {$order2} {$pages->limit}", "", "vending.*, char.name as merchant_name, char.last_map, char.last_x, char.last_y");
	...
	{
		$count = $db->select("vending","", "", "count(*) as cnt");
		$pages->items_total = $count[0]->cnt;
		$pages->mid_range = 5;  
		$pages->paginate();	
		
		$results = $db->select("vending left join `char` on `char`.char_id=`vending`.char_id ORDER BY {$order} {$order2} {$pages->limit}","", "", "vending.*, char.name as merchant_name, char.last_map, char.last_x, char.last_y");
		$totalrows = count($results);
	}
?>
<head>
