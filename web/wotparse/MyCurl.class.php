...
class MyCurl {
	...
	function __construct($url, $cookies='',  $encoding='UTF-8', $raw=false, $post='') {
    	$this->url = $url;
    	$this->cookies = $cookies;
		$this->post = $post;
    	$this->raw = $raw;
    	$this->encoding = $encoding;
    	$this->user_agent = $GLOBALS['scrubber_xml']->user_agent;
	}
	
	function getData()
	{
		if ($GLOBALS['scrubber_xml']->verbose>0) echo "( Downloading ".$this->url . ' )' . PHP_EOL;
		$ch = curl_init();
		...
		if(!empty( $this->post ))
		{
			...
			curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
			foreach($this->post as $key => $value) { $post_fields[] = $key."=".$value; }
